...
class Linear(tf.keras.layers.Layer):
    def __init__(self, units=32, input_dim=32):
        ...
        self.custom_list = [1, 2, 2, 1, 0, 1, 1, 3, 2, 0, 0, 1, 3, 3, 0, 1, 2, 4, 3, 1, 3, 3, 1, 2, 3]

    def call(self, sem, mask):
        t1=tf.multiply(mask, tf.matmul(sem, self.w1)) + self.b1
        tempsem=tf.roll(sem, shift=1, axis=1)
        t2=tf.multiply(mask, tf.matmul(tempsem, self.w1)) + self.b1
        tempsem=tf.roll(sem, shift=-1, axis=1)
        t3=tf.multiply(mask, tf.matmul(tempsem, self.w1)) + self.b1
        tempsem=tf.roll(sem, shift=1, axis=2)
        t4=tf.multiply(mask, tf.matmul(tempsem, self.w1)) + self.b1
        tempsem=tf.roll(sem, shift=-1, axis=2)
        t5=tf.multiply(mask, tf.matmul(tempsem, self.w1)) + self.b1
        tempsem=tf.roll(sem, shift=1, axis=1)
        ...
        t10=tf.multiply(mask, tf.matmul(tempsem, self.w2)) + self.b2
        tempsem=tf.roll(sem, shift=-2, axis=1)
        ...
        t14=tf.multiply(mask, tf.matmul(tempsem, self.w2)) + self.b2
        tempsem=tf.roll(sem, shift=2, axis=1)
        tempsem=tf.roll(tempsem, shift=1, axis=2)
        ...
        tempsem=tf.roll(sem, shift=-1, axis=1)
        tempsem=tf.roll(tempsem, shift=2, axis=2)
        t22=tf.multiply(mask, tf.matmul(tempsem, self.w2)) + self.b2
        tempsem=tf.roll(sem, shift=1, axis=1)
        tempsem=tf.roll(tempsem, shift=2, axis=2)
        t23=tf.multiply(mask, tf.matmul(tempsem, self.w2)) + self.b2
        tempsem=tf.roll(sem, shift=1, axis=1)
        tempsem=tf.roll(tempsem, shift=-2, axis=2)
        t24=tf.multiply(mask, tf.matmul(tempsem, self.w2)) + self.b2
        tempsem=tf.roll(sem, shift=-1, axis=1)
        tempsem=tf.roll( tempsem, shift=-2, axis=2)
        t25=tf.multiply(mask, tf.matmul(tempsem, self.w2)) + self.b2
        sum = 0
        sum = sum + tf.keras.layers.Activation('tanh')(t1) + tf.keras.layers.Activation('relu')(t2) + tf.keras.layers.Activation('relu')(t3) + tf.keras.layers.Activation('tanh')(t4) + tf.keras.layers.Activation('sigmoid')(t5) + tf.keras.layers.Activation('tanh')(t6) + tf.keras.layers.Activation('tanh')(t7) + tf.keras.layers.Activation('softmax')(t8) + tf.keras.layers.Activation('relu')(t9) + tf.keras.layers.Activation('sigmoid')(t10) + tf.keras.layers.Activation('sigmoid')(t11) + tf.keras.layers.Activation('tanh')(t12) + tf.keras.layers.Activation('softmax')(t13) + tf.keras.layers.Activation('softmax')(t14) + tf.keras.layers.Activation('sigmoid')(t15) + tf.keras.layers.Activation('tanh')(t16) + tf.keras.layers.Activation('relu')(t17) + tf.keras.layers.Activation('softmax')(t19) + tf.keras.layers.Activation('tanh')(t20) + tf.keras.layers.Activation('softmax')(t21) + tf.keras.layers.Activation('softmax')(t22) + tf.keras.layers.Activation('tanh')(t23) + tf.keras.layers.Activation('relu')(t24) + tf.keras.layers.Activation('softmax')(t25)
