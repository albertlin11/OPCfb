...
from prof_input_1_feedback import *
from prof_input_1_feedback_cross_a import *
from prof_input_1_feedback_self_a import *
...
parser = argparse.ArgumentParser("main_610_repo")
...
# make result dir
if not os.path.isdir("reload_result"):
    os.makedirs("reload_result")
...
def soft_round_bce(y_true, y_pred):
...
def test_mse(y_true, y_pred): # normal mse
...
def test_mbce(y_true, y_pred): # modified bce
...
def test_mmse(y_true, y_pred): # modified mse
...
def test_sbce(y_true, y_pred): # soft_round the pred before calculate bce
...
def test_msbce(y_true, y_pred): # modified bce
...
def SaveTestLossData(path, results, history=None, loss_fun="default"):
    ...
    # save test loss
    y_pred = results.copy()
    test_bce_score = test_bce(y_true, y_pred)
    f.write("test_bce" + '\n')
    f.write(str(test_bce_score) + '\n')

    # save test loss with MSE
    y_pred = results.copy()
    test_mse_score = test_mse(y_true, y_pred)
    f.write("test_mse" + '\n')
    f.write(str(test_mse_score) + '\n')

    # save test loss with modified BCE
    y_pred = results.copy()
    test_mbce_score = test_mbce(y_true, y_pred)
    f.write("test_mbce" + '\n')
    f.write(str(test_mbce_score) + '\n')

    # save test loss with modified MSE
    y_pred = results.copy()
    test_mmse_score = test_mmse(y_true, y_pred)
    f.write("test_mmse" + '\n')
    f.write(str(test_mmse_score) + '\n')
    ...
    # save BBCE 
    y_pred = results.copy()
    test_sbce_score = test_sbce(y_true, y_pred)
    f.write("test_sbce" + '\n')
    f.write(str(test_sbce_score) + '\n')

    # save modified BBCE
    y_pred = results.copy()
    test_msbce_score = test_msbce(y_true, y_pred)
    f.write("test_msbce" + '\n')
    f.write(str(test_msbce_score.numpy()) + '\n') 
    ...
    # save training mbbce
    f.write("train_val_sbce" + '\n')
    f.write(str(history.history['val_loss'][-1]) + '\n')

    # save training loss mbbce
    f.write("train_mbbce" + '\n')
    f.write(str(history.history['test_msbce'][-1]) + '\n')

    # save training val loss mbbce
    f.write("train_val_mbbce" + '\n')
    f.write(str(history.history['val_test_msbce'][-1]) + '\n')
...
# setting the path
result_filename = args.model + '_'  + str(args.lr) + '_' + str(args.epoch) + '_' + str(args.batch_size) + '_' + str(args.edge_enhanced) + '_' + str(args.seed) + '_' + str(args.custom_list) + '_' + str(args.trial) + '_' + str(args.patience)
path = args.result_dir + '/' + result_filename
check_result_path = "result" + '/' + "M" + args.augmode + "_T" + str(args.augtranslation) + '_R' + str(args.augrotate) + '_S' + str(args.seed) + '_P' + str(args.patience) + '_V' + str(args.version)
...
# model compile
model.compile(optimizer = Adam(learning_rate = args.lr), loss = soft_round_bce, metrics = [test_msbce, PA], run_eagerly=True)
...
# load the model weight if the model is already trained before
model_path = check_result_path + "/" + result_filename + "/" + model_name_bce
#================================================================================================
#                                       train mode
#================================================================================================
...
if args.mode == "train":
    ...
    if os.path.isfile(model_path):
        ...
        print("="*100)
        # change the filename to avoid the name conflict
        if args.model == "unet" and args.trial == 5:
            # change the filename to avoid the name conflict
            os.rename(check_result_path, check_result_path + "_loaded")
