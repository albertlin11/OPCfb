...
def unet_2d_base(input_tensor, filter_num, stack_num_down=2, stack_num_up=2, 
    ...
    X_skip = [] # X_skip is the list to store the result after layer
    depth_ = len(filter_num)

    # no backbone cases
    if backbone is None: #left side
    ...
    X_decode = X_skip[1:] # the result after 512, 256, 128, 64
    depth_decode = len(X_decode)

    # reverse indexing filter numbers
    filter_num_decode = filter_num[:-1][::-1]  #e.g.( 64, 128, 256, 512, 1024) => (512, 256, 128, 64) ie. pop the last one and reverse the rest
    ...
    if depth_decode < depth_-1: # won't enter
...
def unet_2d(input_size, filter_num, n_labels, stack_num_down=2, stack_num_up=2,
    ...
    if backbone is not None: # fool proofing
        bach_norm_checker(backbone, batch_norm)
        
    IN = Input(input_size) #input layer
    #IN.shape = (None, (input_size))
