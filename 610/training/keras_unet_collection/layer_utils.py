...
def attention_gate(X, g, channel,  
    ...
    '''
    # note: channel = f//2 = [128, 64, 32]
    activation_func = eval(activation)
    attention_func = eval(attention)
    # size of X = (channel, (image_size))
    # mapping the input tensor to the intermediate channel
    theta_att = Conv2D(channel, 1, use_bias=True, name='{}_theta_x'.format(name))(X) # X is X_left
    # size of theta_att = (channel, (image_size)),thought as key
    
    
    # mapping the gate tensor
    phi_g = Conv2D(channel, 1, use_bias=True, name='{}_phi_g'.format(name))(g) # g is the result right now
    # size of theta_att = (channel, (image_size)),thought as query
    
    # ----- attention learning ----- #
    query = attention_func([theta_att, phi_g], name='{}_add'.format(name)) # add function will find the attention between phi_g and theta_att
    # size of query = (channel,(image_size))
    
    # nonlinear activation
    f = activation_func(name='{}_activation'.format(name))(query) #ReLU
    
    # linear transformation
    psi_f = Conv2D(1, 1, use_bias=True, name='{}_psi_f'.format(name))(f) # size of psi_f = (1, (image_size))
    # ------------------------------ #
    
    # sigmoid activation as attention coefficients 
    coef_att = Activation('sigmoid', name='{}_sigmoid'.format(name))(psi_f) # set the value to 0 ~ 1
    #coef_att is attention score
    #size of coef_att = (1,(image_size))
    
    
    # multiplicative attention masking
    X_att = multiply([X, coef_att], name='{}_masking'.format(name))
    # the size of X_att = (channel, (image_size))
...
def CONV_stack(X, channel, kernel_size=3, stack_num=2, 
    ...
    for i in range(stack_num):
        ...
        activation_func = eval(activation) #eval will return the function of the string
