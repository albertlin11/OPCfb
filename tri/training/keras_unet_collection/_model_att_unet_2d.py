...
def UNET_att_right(X, X_left, channel, att_channel, kernel_size=3, stack_num=2,
    ...
    # X = (512, iamge_size)
    # X_left = (256, image_size)
    # channel = [256, 128, 64]
    # att_channel = [128, 64, 32]
...
def att_unet_2d_base(input_tensor, filter_num, stack_num_down=2, stack_num_up=2,
    ...
    X = X_skip[0] # the first one store to X
    # other tensors are preserved for concatenation
    X_decode = X_skip[1:] # those after the first one store to X_decode
    depth_decode = len(X_decode) 

    # reverse indexing filter numbers
    filter_num_decode = filter_num[:-1][::-1] #e.g.[ 64, 128, 256, 512] => [256, 128, 64] ie. pop the last one and reverse the rest

    for i in range(depth_decode): #depth_decode = 3
