            ...
            for  ROTATE in "${ROTATE_LIST[@]}"
                ...
                do
                    ...
                    do
                        ...
                        do
                            python main_two_repo_prof.py --model ${MODEL}  --trial "$i" --GPU ${GPU}  --result_dir ${RESULT_DIR} --augmode ${AUGMODE} --patience ${PATIENCE} --augrotate ${ROTATE} --augtranslation ${TRANSLATION} --seed ${SEED} --epoch ${EPOCH} --version ${VERSION}
                    ...
                    python get_rank.py
                    python get_table.py
                    mv csv_record.csv two_${FILENAME}_record.csv
                    mv avg_record.csv two_${FILENAME}_avg.csv
                    mv rank_record.csv two_${FILENAME}_rank.csv
                    mv table_record.csv two_${FILENAME}_table.csv
                    mv two_${FILENAME}_record.csv ${RESULT_DIR}
                    mv two_${FILENAME}_avg.csv ${RESULT_DIR}
                    mv two_${FILENAME}_rank.csv ${RESULT_DIR}
                    mv two_${FILENAME}_table.csv ${RESULT_DIR}
                    mv ${RESULT_DIR} ${FILENAME}  
                    cp main_two_repo_prof.py ${FILENAME}/main.py
