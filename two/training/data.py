...
def saveResult(path, npyfile, loss_fun = "default"):
...
def saveDiff(path, npyfile, loss_fun = "default"):
    for i,item in enumerate(npyfile):
        filename = os.path.join(path,"%04d_difference_%s.png"%(i, loss_fun))
        cv2.imwrite(filename, cv2img)


def SaveFigureBy0to1(path,npyfile, name="difference", loss_fun = "default"):
    figure_name = "%04d_" + name + "_" + loss_fun + ".png"
    for i, item in enumerate(npyfile):
        cv2img = item * 255
        cv2.imwrite(os.path.join(path,figure_name%i), cv2img)
        
def SaveFigureBy0to255(path,npyfile, loss_fun = "default"):
    figure_name = "%04d_difference_" + loss_fun + ".png"
    for i, item in enumerate(npyfile):
